...
public class PageObjectModelGeneratorV2 {
...
//	   landingPage.ClickQueueLnk();
	   ClientPageModel clientPage = landingPage.ClickClientLnk();
	   clientPage.GetNewLnk().click();

	   PageObjectModelGeneratorV2.PrintPageModelCode(driver);
	}
	
	public void findAndClickCBorInterimApplication(WebDriver driver) {
			...
			for(Iterator<WebElement> iInner = iElementList.iterator(); iInner.hasNext();) {
				...
				while(propNameFound==false) {
					...
					else if (propName.substring(0, 3).equals("ddl"))//select
					{
						...
						ActionMethodsDefinitions += String.format(""
						+"\npublic ThisClassName SelectOption%s(WebDriver driver, String originalWindow, String optionText)",propNameUpper) + String.format(""
						+"\n{"
						+"\n\tWebElement selectElement = this.GetAttendingSchoolSelect();", propNameUpper)
